...
task :load_legacy_location_data => :environment do
    ...
    require 'json'
    records = Trailer.all
    urls = ['https://www.kalkaskalibrary.org/wp-content/uploads/json/parsed-hours.json', 'https://www.sbbdl.org/wp-content/uploads/json/parsed-hours.json']
    urls.each do |url|
        response = JSON.parse(open(url).read) rescue nil
        response["locations"].each do |l|
            location = Location.new
            location.shortname = l['shortname']
            location.group = l['shortname']
